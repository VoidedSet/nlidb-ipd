...
// Global Groq client (always uses server .env key)
...
const generateSQLQuery = async (databaseName, schema, chatHistory, userQuery) => {
    ...
    const prompt = `
        ...
        The database name is "${databaseName}". Based on the schema below, write a SQL query that answers the question. 
        ...
        Write only the SQL query and nothing else. No backticks.
...
const formatSQLResponse = async (sqlQuery, result, error = null) => {
    ...
    const prompt = `
        ...
        If there is an error, explain it simply and show how to fix it.

        Replace code blocks with <code> tags.
        Use <p>, <br>, <bold>, <strong>, and HTML tables for readability.
    ...
    try {
        ...
        if (!db) {
            return res.status(400).json({
                sqlQuery: null,
                formattedResponse: "No database connected. Please provide credentials first.",
                error: true
            });
        }
        ...
        res.status(500).json({
            sqlQuery: null,
            formattedResponse: "Internal Server Error. Please try again later.",
            error: true
        });
    } finally {
        if (db) db.end();
